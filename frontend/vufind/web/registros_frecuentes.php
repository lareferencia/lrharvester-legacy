...
$url = 'http://200.0.206.180:8080/solr/stats/select?q='.urlencode('recordId:[* TO *]').'&wt=xml&facet=true&facet.field=recordId&fl=0&facet.limit=10&rows=0';
...
echo '<body>';
echo '<h1>Registros consultados m&aacute;s frecuentes.</h1>';
echo '<ul>';
foreach ($xml->xpath("//lst[@name='recordId']/int") as $busqueda) {
    echo '<li><a href="http://200.0.206.180/vufind/Record/',$busqueda['name'],'">',$busqueda,'-',$busqueda['name'],'</a></li>',PHP_EOL;
}
echo '</ul>';
echo '</body>';
echo '</html>';
