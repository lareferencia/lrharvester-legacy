...
class CitationBuilder
{
	    ...
	    public function getAPA_T()
    {
        ...
        $apa['periodAfterTitle']
            = (!$this->_isPunctuated($apa['title']) && empty($apa['edition']));
        return $apa['authors'].$apa['year'].'. '.$apa['title'].'. '.$apa['edition'].'. '.$apa['publisher'].'. URL:'.$apa['url'];
    }
    /**
     * Get MLA citation.
     *
    ...
    {
        if (isset($this->_details['elurl'])) {
            return $this->_details['elurl'];
        }
        return false;
    }
