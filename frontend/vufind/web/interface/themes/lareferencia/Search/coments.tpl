<div class="span-18{if $sidebarOnLeft} push-5 last{/if}">
    {if !$noHistory}
      {if $saved}
        <h3>{translate text="Comentarios Realizados"}</h3>
        <table class="datagrid" width="100%">
          <tr>
            <th width="25%">{translate text="Registro"}</th>
            <th width="30%">{translate text="Comentario"}</th>
            <th width="30%">{translate text="Fecha"}</th>
          </tr>
          ...
          <tr class="oddrow">
          {/if}
            <td><a href="{$path}/Record/{$info.record_id}">{$info.record_id}</a></td><td>{$info.comment}</td><td>{$info.created}</td>
           </tr>
          {/foreach}
        </table>
      {/if}
        ...
        <table class="datagrid" width="100%">
          <tr>
            ...
            <th width="10%">{translate text="history_results"}</th>
			 <th width="10%">Actualizaciones</th>
