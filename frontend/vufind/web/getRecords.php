...
if(isset($_GET["url"]))
   $url2=$_GET["url"];
 	...
 	$url2="http://lareferencia.shell.la:8090//public/listInvalidRecordsInfoBySnapshotID/92";
	...
	$output7.=  "<tr> <th>Pa&iacute;s</th><th>ID</th><th>Identificador</th><th>Estatus</th><th>Detalle</th><th></th></tr>";
		foreach($data2 as $red){
			...
			//echo "****************************************************";
				...
				if ($key2==="href")
				{
				$outputN.= "<a style='font-family:Verdana;font-size:8pt;color:#2E8CB4;text-decoration: none' href='http://200.0.206.214/vufind/getRecords.php?iso=".$iso."&url=".$value2."'>".$link."</a>-";
				}
			if ($key2==="id")
				{
				//echo print_r($value2);
