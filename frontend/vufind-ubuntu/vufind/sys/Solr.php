...
*/
    ...
    {
        // Query String Parameters
        $options = array('q' => "oid:$id", 'qt' => 'morelikethis');
        $result = $this->_select('GET', $options);
        if (PEAR::isError($result)) {
            PEAR::raiseError($result);
        }
...
//$this->client->addHeader('Content-Type', 'text/plain; charset=utf-8'); //LAREFERENCIA
                ...
                if ($function != '') {
                    ...
                    if (is_array($value)) {
                        foreach ($value as $additional) {
							...
							$additional = utf8_decode($additional); //LAREFERENCIA
                            $query[] = "$function=$additional";
                        }
                    } else {
                        //$value = urlencode($value);
						$value = utf8_decode($value);
                        $query[] = "$function=$value"; //LAREFERENCIA
                    }
                }
            }
        }
        ...
        //echo "CLIENTE: ";LAREFERENCIA
		//print_r($this->client);LAREFERENCIA
...
//echo "CURL:"."http://200.0.207.91:8080/solr/stats/update?commit=true -H \"Content-Type: text/xml\" --data-binary '".$xml."'" ;
//echo $title;
return true;
