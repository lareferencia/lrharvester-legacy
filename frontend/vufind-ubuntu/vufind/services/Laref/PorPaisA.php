...
class PorPaisA extends Action
{
	...
	$result = mysql_query("SELECT count(*) as acceso,ccode FROM record WHERE ((ccode<>'' OR not(NULL)) AND (type='Artículo')) GROUP BY ccode ORDER BY ccode ") 
	or die(mysql_error());  
	$table="";
	$list="";
...
// Get all the data from the "example" table
$result = mysql_query("SELECT count(*) as total,ccode,type from record WHERE ((ccode<>'' OR not(NULL)) AND (type='Artículo')) group by ccode,type order by total desc") 
or die(mysql_error());  
...
while($row = mysql_fetch_array( $result )) {
	...
	$output6.=$row['total'];
        ...
        $interface->setTemplate('porpaisa.tpl');
        $interface->setPageTitle('Impacto de Art&iacute;culos por Pa&iacute;s');
