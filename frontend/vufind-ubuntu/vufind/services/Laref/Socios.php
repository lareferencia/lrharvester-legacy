...
class Socios extends Action
{
    ...
    public function launch()
    {
		...
		$url="http://lareferencia.shell.la:8090/listNetworks";
		...
		foreach($data as $red){
			foreach($red as $key => $value){
			  if ($key==="name")
				$output1 .=  "Red : $value<br>";
			  if ($key==="datestamp")
				$output1 .= "Fecha de &uacute;ltima actualizaci&oacute;n: $value<br>";
			  if ($key==="size")
				$output1 .= "registros colectados : ".number_format((int)$value)."<br>";
			  if ($key==="validSize")
				$output1 .= "registros validos : ".number_format((int)$value)."<br>";
			}
		}
