...
class PorPaisR extends Action
{
	...
	$result = mysql_query("SELECT count(*) as acceso,ccode FROM record WHERE ((ccode<>'' OR not(NULL)) AND (type='Reporte')) GROUP BY ccode ORDER BY ccode ") 
	or die(mysql_error());  
	$table="";
	$list="";
...
// Get all the data from the "example" table
$result = mysql_query("SELECT count(*) as total,ccode,type from record WHERE ((ccode<>'' OR not(NULL)) AND (type='Reporte')) group by ccode,type order by total desc") 
or die(mysql_error());  
...
while($row = mysql_fetch_array( $result )) {
	...
	$output6.=$row['total'];
        ...
        $interface->setTemplate('porpaisr.tpl');
        $interface->setPageTitle('Impacto de Reporte por Pa&iacute;s');
