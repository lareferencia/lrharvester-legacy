 ...
 * Type:     function
 * Name:     image
 * Purpose:  Loads an image file from the appropriate theme
 *           directory.  Supports one parameter:
 *              filename (required) - file to load from
 *                  interface/themes/[theme]/js/ folder.
 *           Any other parameters are added as attributes to
 ...
 *
 * @return string        <img> tag for including the image
...
{   // @codingStandardsIgnoreEnd
    ...
    foreach ($themes as $theme) {
        ...
        if (file_exists("{$local}/interface/themes/{$theme}/images/{$filename}")) {
            $image = "{$path}/interface/themes/{$theme}/images/{$filename}";
            break;
        }
    }

    // If we couldn't find the file, check the global image area; if that
