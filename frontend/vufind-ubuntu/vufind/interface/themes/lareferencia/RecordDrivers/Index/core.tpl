<div class="span-13">
  ...
  <table cellpadding="2" cellspacing="0" border="0" class="citation" summary="{translate text='Bibliographic Details'}">
    ...
    <tr valign="top">
      ...
      <td><a href="{$url}/Search/Results?lookfor=%22{$coreMainAuthor|escape:"url"}%22">{$coreMainAuthor|escape}</a></td>
    </tr>
    {/if}
    ...
    <tr valign="top">
      ...
      <td><a href="{$url}/Search/Results?lookfor=%22{$coreCorporateAuthor|escape:"url"}%22">{$coreCorporateAuthor|escape}</a></td>
    </tr>
    {/if}
    ...
    <tr valign="top">
      ...
      <td>
        {foreach from=$coreContributors item=field name=loop}
          <a href="{$url}/Search/Results?lookfor=%22{$field|escape:"url"}%22">{$field|escape}</a>{if !$smarty.foreach.loop.last}, {/if}
        {/foreach}
      </td>
    </tr>
    {/if}
    ...
    <tr valign="top">
      ...
      <td>
        {foreach from=$coreURLs item=desc key=currentUrl name=loop}
          <a onclick="return record_outbound(this.href,'{$coreId|escape}');" target="_blank" href="{if $proxy}{$proxy}/login?qurl={$currentUrl|escape:"url"}{else}{$currentUrl|escape}{/if}">{$desc|escape}</a><br/>
        {/foreach}
        {if $coreOpenURL}
...
<div class="span-4 last">
  ...
  {else}
		{if $coreCountryISO}
			<img src="{$path}/images/{$coreCountryISO|escape}icon.gif" class="recordcover" alt="{translate text='Pais'}"/><br/>
			{else}
