<div class="result recordId" id="record{$summId|escape}">
  ...
  <div class="span-2">
    {if $summCountryISO}
    <img src="{$path}/images/{$summCountryISO|escape}icon.gif" class="summcover" alt="{translate text='Pais'}"/>
    {else}
  ...
  <div class="span-9">
    <div class="resultItemLine1">
      <a href="{$url}/Record/{$summId|escape:"url"}" class="title">{$summTitle|escape}</a>
    </div>
    ...
    <div class="last">
      ...
      {if $summOpenUrl || !empty($summURLs)}
        ...
        {foreach from=$summURLs key=recordurl item=urldesc}
          <br/><a href="{if $proxy}{$proxy}/login?qurl={$recordurl|escape:"url"}{else}{$recordurl|escape}{/if}" class="fulltext" target="_blank" onclick="return record_outbound(this.href,'{$summId|escape}');">{if $recordurl == $urldesc}{translate text='Get full text'}{else}{$urldesc|escape}{/if}</a>
        {/foreach}
      {/if}
