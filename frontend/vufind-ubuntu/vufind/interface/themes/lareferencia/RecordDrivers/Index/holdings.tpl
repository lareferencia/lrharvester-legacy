...
{if !empty($holdingURLs) || $holdingsOpenURL}
  ...
  {if !empty($holdingURLs)}
    {foreach from=$holdingURLs item=desc key=currentUrl name=loop}
      <a onclick="return record_outbound(this.href,'{$holdingId|escape}');" target="_blank" href="{if $proxy}{$proxy}/login?qurl={$currentUrl|escape:"url"}{else}{$currentUrl|escape}{/if}">{$desc|escape}</a><br/>
    {/foreach}
  {/if}
  {if $holdingsOpenURL}
