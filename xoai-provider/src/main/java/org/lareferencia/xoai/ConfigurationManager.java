 ...
 * <code>xoai.configuration</code> should be set to the <em>filename</em>
 * of the configuration file.
 * <P>
 * Other configuration files are read from the <code>config</code> directory
 * of the xoai installation directory (specified as the property
 * <code>xoai.dir</code> in the main configuration file.)
 *
 *
...
public class ConfigurationManager
{
    ...
    /**
     * Identify if xoai is properly configured
    ...
    /**
     * REMOVED - Flushing the properties could be dangerous in the current xoai state
...
//    }
   ...
   /**
    ...
    /**
     * Returns an enumeration of all the keys in the xoai configuration
     *
     * @return an enumeration of all the keys in the xoai configuration
     */
    ...
    private static synchronized void loadModuleConfig(String module)
    {
        ...
        try
        {
            modFile = new File(getProperty("xoai.dir") +
    ...
    /**
     * Load the xoai configuration properties. Only does anything if
     * properties are not already loaded. Properties are loaded in from the
     * specified file, or default locations.
     *
     * @param configFile
     *            The <code>xoai.cfg</code> configuration file to use, or
    ...
    public static synchronized void loadConfig(String configFile)
    {
        ...
        try
        {
            ...
            try
            {
                configProperty = System.getProperty("xoai.configuration");
            }
            ...
            {
                info("Loading system provided config property (-Dxoai.configuration): " + configProperty);
            ...
            {
                url = ConfigurationManager.class.getResource("/xoai.cfg");
                if (url != null)
                {
            ...
            if (url == null)
            {
                fatal("Cannot find xoai.cfg");
                throw new IllegalStateException("Cannot find xoai.cfg");
            }
            else
            {
     ...
     * <code>name</code> from <code>xoai.cfg</code> to the standard
    ...
    public static void main(String[] argv)
    {
        ...
        {
            System.err
                    .println("Usage: ConfigurationManager OPTION\n  [-module mod.name] -property prop.name  get value of prop.name from module or xoai.cfg");
        }
