...
public interface NetworkSnapshotRepository extends JpaRepository<NetworkSnapshot, Long> { 
	  ...
	  @Query("select ns from NetworkSnapshot ns where ns.network.id = ?1 and ns.status = 4 and ns.endTime >= (select max(s.endTime) from NetworkSnapshot s where s.network.id = ?1 and s.status = 4)")
