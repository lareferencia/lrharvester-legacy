...
public class SnapshotWorker implements ISnapshotWorker, IHarvestingEventListener {
	...
	// No son autowired, se cargan en cada ejecución de acuerdo a la conf de
	// cada red
	...
	/**
	 * TODO: Podría ser Async, pero no tiene sentido empezar un nuevo proceso de
	 * harvesting para una misma red si el anterior no terminó. Hay que cuidar
	 * los bloqueos!!! TODO: Podría implemetarse la limpieza de procesos
	 * inactivos para evitar problemas
	 * 
	 * @throws Exception
	 */
	@Override
	public void run() {
		...
		if (network.mustRunValidation()) {
			...
			// Se cargan el validador y el transformador de acuerdo a la
			...
			try {
				...
				// validatorManager.createModelFromValidator(validator,
				// "Validador Base", "Un validador de test");
				//
		...
		if (snapshot.getStatus() != SnapshotStatus.HARVESTING_STOPPED) {
			...
			if (snapshot.getStatus() != SnapshotStatus.HARVESTING_FINISHED_ERROR) {
				...
				if (network.isPublished()) {
					...
					// si no está publicada o no se indexa la marca como válida
					// sin indexar
					// Graba el status
					setSnapshotStatus(SnapshotStatus.VALID);
				}
				...
				try {
					...
					if (network.mustRunValidation()) {
						...
						if (network.mustRunTransformation()) {
							...
							// Si es válido y hubo transformación incrementa la
							// cuenta de válidos transformados
							if (wasTransformed && validationResult.isValid())
								snapshot.incrementTransformedSize();

						}

						// registra si resultó válido o es irrecuperable
						// (inválido)
					...
					else { // Si no se valida entonces todo puesto para
							// publicación
							// TODO: Chequear esto
