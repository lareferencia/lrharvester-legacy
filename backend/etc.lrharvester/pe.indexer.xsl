...
<xsl:stylesheet version="1.0"
	...
	<xsl:variable name="peru_prefix">info:pe-repo/semantics/</xsl:variable> 
    ...
    <xsl:template match="oai_dc:dc">
            <doc>
               ...
               <xsl:for-each select="//dc:type">
                   ...
                   <!--  CASO DE PE-REPO TIPOS TEMPORALES  -->
                   <xsl:if test="starts-with(., $peru_prefix)">
							  <field name="format">
                           			<xsl:value-of select="substring(., string-length($peru_prefix)+1, string-length(.))" />
                       		  </field>           
                   </xsl:if>
               </xsl:for-each>
